...
registerHelper('if', function (parent, context, expression, body, alternate) {
    ...
    body(trueDiv, context);
    alternate(falseDiv, context);
...
registerHelper('documentFragment', function (parent, context, expression) {
    var anchor = document.createComment('document fragment placeholder');
...
registerHelper('with', function (parent, context, expression, body) {
    context = context.clone();
    context.path = context.path + expression.keypath + '.';
    body(parent, context);
});
